  ...
  return (
    <div className="glass rounded-xl p-3">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-[11px] text-white/40 uppercase tracking-wider">{t('groups.group', { l: letter })}</span>
      </div>
      ...
      <table className="w-full text-[11px]">
        <thead>
          <tr className="text-white/30 text-[9px] uppercase">
            <th className="text-left font-medium pb-1">{t('groups.team')}</th>
            <th className="font-medium pb-1 w-6">{t('groups.pj')}</th>
            <th className="font-medium pb-1 w-6">{t('groups.gd')}</th>
            <th className="font-medium pb-1 w-6 text-right">{t('groups.pts')}</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r, i) => (
      ...
      {fixtures?.length > 0 && (
        <div className="mt-2.5 pt-2 border-t border-white/[0.06]">
          <div className="text-[8px] uppercase tracking-wider text-white/30 mb-1">{t('groups.matches')}</div>
  ...
  return (
    <div className="p-4 sm:p-5">
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 mb-4 text-[10px] text-white/40">
        <p className="text-xs text-white/45">{tr('groups.intro')}</p>
        ...
        <span className="flex items-center gap-1"><span className="text-emerald-400">✓</span>/<span className="text-red-400/80">✗</span> {t('groups.legendResult')} · <span className="tracking-[0.15em]">···</span> {t('groups.legendPending')}</span>
      </div>
